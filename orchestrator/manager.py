...
class Job:
    ...
    def from_dict(cls, data: Dict) -> 'Job':
        """Create a Job from a plan dictionary."""
        job_id = data.get('id', f'job_{int(time.time())}')
        description = data.get('description', 'Execute task')
        ...
        for s in steps_data:
            if isinstance(s, dict):
                step = JobStep(
                    type=s.get('type', s.get('action', 'unknown')),
                    params=s.get('params', s.get('parameters', {}))
                )
                steps.append(step)
            elif isinstance(s, JobStep):
                steps.append(s)
        
        return cls(id=job_id, description=description, steps=steps)
